 ...
 * <h1>JVM Metrics</h1>
 * <ol>
 * <li>The number of classes that are currently loaded in the Java Virtual
 * Machine</li>
 * <li>The total number of classes that have been loaded since the JVM has
 * started execution</li>
 * <li>The total number of classes unloaded since the JVM has started
 * execution</li>
 * </ol>
 *
...
final class ClassLoaderMetrics {
    ...
    private static final String JAVA_VIRTUAL_MACHINE_HAS_STARTED_EXECUTION = "Java Virtual Machine has started execution.";
    private static final String UNIT_CLASSES = "classes";

    void bindTo(final MetricRegistry registry, final boolean micrometerFormat) {
        // The ClassLoadingMXBean can be used in native mode, but it only returns
        // zeroes, so there's
        // no point in
        ...
        if (micrometerFormat) {
            registry.register(
                    new ExtendedMetadataBuilder().withName("jvm.classes.loaded").withType(MetricType.GAUGE)
            ...
            registry.register(
                    new ExtendedMetadataBuilder().withName("jvm.classes.loaded.total").withType(MetricType.GAUGE) // should
                                                                                                                  // be
                                                                                                                  // a
                                                                                                                  // counter?!
                            .withUnit(UNIT_CLASSES)
            ...
            registry.register(new ExtendedMetadataBuilder().withName("jvm.classes.unloaded")
                    .withType(MetricType.COUNTER).withUnit(UNIT_CLASSES)
                    .withDescription(
                    ...
                    .skipsScopeInOpenMetricsExportCompletely(true).build(), new GetCountOnlyCounter() {
            ...
            registry.register(new MetadataBuilder()
                    // required by MP spec
                    .withName("classloader.loadedClasses.count").withDisplayName("Current Loaded Class Count")
                    .withType(MetricType.GAUGE).withDescription("""
                            Displays the number of classes that are currently loaded in the \
                            Java Virtual Machine\
                            """).build(), (Gauge<?>) classLoadingBean::getLoadedClassCount);

            registry.register(new MetadataBuilder()
                    // required by MP spec
                    .withName("classloader.loadedClasses.total").withType(MetricType.COUNTER)
            ...
            registry.register(new MetadataBuilder()
                    // required by MP spec
                    .withName("classloader.unloadedClasses.total").withDisplayName("Total Unloaded Class Count")
